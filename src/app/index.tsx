            ...
            {reviews.filter(review => !social.blocked.includes(review.user_id ?? '')).map((review) => (
              <View key={review.id} style={styles.reviewCard}>
                <View style={styles.userRow}>
                  ...
                  <View style={styles.userInfo}><Text style={styles.username} numberOfLines={1}>{review.full_name?.trim() || review.username || CURRENT_USERNAME}</Text><Text style={styles.handle} numberOfLines={1}>@{review.username || CURRENT_USERNAME}</Text><Text style={styles.date}>{formatDate(review.createdAt)}</Text></View>
...
const baseStyles = StyleSheet.create({
  ...
  postCard: { backgroundColor: '#0F1016', borderRadius: 16, padding: 14, marginBottom: 12, borderWidth: 1, borderColor: '#24252D', width: '100%', maxWidth: '100%', minWidth: 0, alignSelf: 'stretch' },
  reviewPostCard: { borderColor: '#34284F', backgroundColor: '#111018' },
  quotePostCard: { borderLeftWidth: 3, borderLeftColor: '#8D65F2', backgroundColor: '#111017' },
