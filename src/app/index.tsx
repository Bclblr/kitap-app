...
export default function HomeScreen() {
  ...
  const loadPosts = useCallback(async () => {
    ...
    try {
      ...
      const reviewPosts: Post[] = (reviewData ?? []).map((review: any) => ({
        ...
        username:
          profilesByUserId.get(review.user_id)?.username || CURRENT_USERNAME,
    ...
    try {
      ...
      if (storyUserIds.length > 0) {
        ...
        storyProfiles = new Map(
          (storyProfileData ?? []).map((profile: any) => [profile.id, profile])
        );
      }
    ...
    try {
      ...
      if (postImage) {
        ...
        if (!arrayBuffer || arrayBuffer.byteLength === 0) {
          throw new Error('Fotoğraf dosyası boş.');
        }
        ...
        console.log('POST FOTOĞRAF URL:', imageUrl);
      }
      ...
      Alert.alert(
        'Hata',
        error instanceof Error ? error.message : 'Gönderi paylaşılırken hata oluştu.'
      );
    } finally {
      setPosting(false);
    }
  ...
  async function createStory() {
    ...
    try {
      ...
      if (storyImage) {
        ...
        if (!response.ok) {
          throw new Error('Hikâye fotoğrafı dosyası okunamadı.');
        }

        const arrayBuffer = await response.arrayBuffer();

        if (!arrayBuffer || arrayBuffer.byteLength === 0) {
          throw new Error('Hikâye fotoğrafı dosyası boş.');
        }
        ...
        imageUrl = publicUrlData.publicUrl;
        console.log('STORY FOTOĞRAF URL:', imageUrl);
      }
      ...
      if (error) {
        ...
        if (imageUrl) {
          ...
          if (fileName) {
            const filePath = `${user.id}/${fileName}`;
            await supabase.storage.from('story-images').remove([filePath]);
          }
        }
      ...
      if (data) {
        setStories((current) => [data as Story, ...current]);
      }
      ...
      Alert.alert(
        'Hata',
        error instanceof Error ? error.message : 'Hikâye paylaşılırken hata oluştu.'
      );
    } finally {
  ...
  async function toggleSavePost(post: Post) {
    ...
    try {
      if (post.saved) {
        ...
        setPosts((current) =>
          current.map((item) =>
            item.id === post.id ? { ...item, saved: false } : item
          )
        );
      } else {
        const { error } = await supabase
        ...
        setPosts((current) =>
          current.map((item) =>
            item.id === post.id ? { ...item, saved: true } : item
          )
        );
      }
    } catch (error) {
  ...
  async function togglePostLike(post: Post) {
    ...
    if (!user) {
      Alert.alert('Giriş gerekli', 'Gönderiyi beğenmek için önce giriş yapmalısın.');
      return;
    }
    ...
    try {
      ...
      if (likeError) {
        Alert.alert('Hata', likeError.message);
        return;
      }
      ...
      setPosts((current) =>
        current.map((item) =>
          item.id === post.id
            ? { ...item, liked: true, likes: (item.likes ?? 0) + 1 }
            : item
        )
      );
      ...
      if (postError || !postData) {
        console.error('Post sahibi bulunamadı:', postError);
        return;
      }
      ...
      if (notificationCheckError) {
        console.error('Beğeni bildirimi kontrol edilemedi:', notificationCheckError);
        return;
      }
      ...
      if (!existingNotification) {
        ...
        if (notificationError && notificationError.code !== '23505') {
          console.error('Beğeni bildirimi oluşturulamadı:', notificationError);
        }
      }
    } catch (error) {
  ...
  async function togglePostRepost(post: Post) {
    ...
    if (!user) {
      Alert.alert('Giriş gerekli', 'Gönderiyi repost etmek için önce giriş yapmalısın.');
      return;
    }
    ...
    try {
          ...
          current.map((item) =>
            item.id === post.id
              ? {
                  ...item,
                  reposted: false,
                  reposts: Math.max(0, (item.reposts ?? 0) - 1),
                }
              : item
          )
        );
      ...
      if (repostError) {
        Alert.alert('Hata', repostError.message);
        return;
      }
      ...
      setPosts((current) =>
        current.map((item) =>
          item.id === post.id
            ? { ...item, reposted: true, reposts: (item.reposts ?? 0) + 1 }
            : item
        )
      );
      ...
      if (postError || !postData) {
        console.error('Repost post sahibi bulunamadı:', postError);
        return;
      }
      ...
      if (notificationCheckError) {
        console.error('Repost bildirimi kontrol edilemedi:', notificationCheckError);
        return;
      }
      ...
      if (!existingNotification) {
        ...
        if (notificationError && notificationError.code !== '23505') {
          console.error('Repost bildirimi oluşturulamadı:', notificationError);
        }
      }
    } catch (error) {
  ...
  async function submitPostComment(postId: string) {
    ...
    try {
      ...
      if (error) {
        console.error('Post yorum hatası:', error);
        Alert.alert('Hata', error.message);
        return;
      }
      ...
      if (postData.user_id !== user.id) {
        ...
        if (notificationError) {
          console.error('Post yorum bildirimi oluşturulamadı:', notificationError);
        }
      }
      ...
      },
      {
        ...
        onPress: async () => {
          try {
            ...
            console.error('Post yorumu silme hatası:', error);
          }
        },
      },
    ]);
  ...
  async function toggleRepost(reviewId: string) {
    try {
      ...
      if (!user) {
        Alert.alert('Giriş gerekli', 'Repost yapmak için önce giriş yapmalısın.');
        return;
      }
  ...
  const activeStoryGroup = storyGroupIndex === null
    ? null
      ...
      >
        ...
        <View style={styles.storySection}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => setShowStoryBox(!showStoryBox)}>
              <Text style={styles.addStoryText}>+ Hikâye</Text>
            </Pressable>
          </View>
          ...
          {showStoryBox && (
            <View style={styles.storyCreateBox}>
              ...
              <View style={styles.createActions}>
                <Pressable onPress={pickStoryImage} style={styles.secondaryButton}>
                  <Text style={styles.secondaryButtonText}>📷 Fotoğraf</Text>
                </Pressable>
                ...
                >
                  <Text style={styles.primarySmallText}>
                    {postingStory ? 'Paylaşılıyor...' : 'Paylaş'}
                  </Text>
                </Pressable>
              </View>
            </View>
                  ...
                  >
                    ...
                    >
                      {group.profile_image || previewStory?.image_url ? (
                        <Image
                          source={{
                            uri: group.profile_image || previewStory?.image_url || '',
                          }}
        ...
        >
          ...
          {showPostBox && (
            <View style={styles.postCreateBox}>
              ...
              <View style={styles.createActions}>
                <Pressable onPress={pickPostImage} style={styles.secondaryButton}>
                  <Text style={styles.secondaryButtonText}>📷 Fotoğraf</Text>
                </Pressable>
        ...
        {loadingPosts ? (
          <View style={styles.loadingBox}>
            <ActivityIndicator />
            <Text style={styles.info}>Gönderiler yükleniyor...</Text>
          </View>
          ...
          <View style={styles.empty}>
            ...
            <Pressable onPress={() => setShowPostBox(true)} style={styles.emptyButton}>
                ...
                >
                    ...
                    <View style={styles.avatar}>
                      {post.profile_image ? (
                        ...
                        <Text style={styles.avatarText}>
                          {post.username?.trim().charAt(0).toUpperCase() || 'K'}
                        </Text>
                      )}
                    </View>
                  ...
                  {post.rating > 0 && (
                    <View style={styles.rating}>
                      ...
                      <Text style={styles.ratingNumber}>{post.rating}/5</Text>
                    </View>
                  )}
                  ...
                  {!post.isQuote &&
                    ...
                    <View style={styles.commentBox}>
                      <TextInput
                        ...
                        onChangeText={post.isReview ? setCommentText : setPostCommentText}
                          ...
                          <View key={comment.id} style={styles.comment}>
                            <View style={styles.commentHeader}>
                              ...
                              {isOwnComment && (
                                ...
                                >
                                  <Text style={styles.deleteComment}>🗑️ Sil</Text>
                                </Pressable>
                              )}
                            </View>
        ...
        {loading ? (
          <Text style={styles.info}>Paylaşımlar yükleniyor...</Text>
        ) : reviews.length > 0 && posts.length === 0 && feedTab === 'for-you' && !social.error ? (
          <>
            <View style={styles.sectionHeader}>
              <Text style={styles.sectionTitle}>Kitap İncelemeleri</Text>
            </View>
            ...
            {reviews.filter(review => !social.blocked.includes(review.user_id ?? '')).map(
              (review) => (
                <View key={review.id} style={styles.reviewCard}>
                  <View style={styles.userRow}>
                    <View style={styles.avatar}>
                      {review.profile_image ? (
                        <Image source={{ uri: review.profile_image }} style={styles.avatarImage} />
                      ) : (
                        <Text style={styles.avatarText}>
                  ...
                  <Pressable
                    onPress={() => openBook(review.bookKey)}
                    style={styles.bookAttachment}
                  >
                    <View style={styles.bookAttachmentIcon}>
                    ...
                    <View style={styles.bookAttachmentInfo}>
                      <Text style={styles.bookAttachmentLabel}>KİTAP</Text>
                      <Text style={styles.bookTitle} numberOfLines={2}>{review.bookTitle}</Text>
                    </View>
                    <Text style={styles.bookAttachmentArrow}>›</Text>
                  </Pressable>
                  ...
                  <View style={styles.actions}>
                    <Pressable
                      ...
                      accessibilityLabel={review.liked ? 'Beğeniyi kaldır' : 'Beğen'}
                    >
                      <Text style={[styles.action, review.liked && styles.likedAction]}>
                        {review.liked ? '♥' : '♡'}
                      </Text>
                      <Text style={styles.count}>{review.likes ?? 0}</Text>
                    </Pressable>

                    <Pressable
                      onPress={() => openCommentBox(review.id)}
                      style={styles.actionButton}
                      accessibilityLabel="Yorumlar"
                    >
                    ...
                    <Pressable
                      onPress={() => toggleRepost(review.id)}
                      style={styles.actionButton}
                      accessibilityLabel={review.reposted ? 'Repostu kaldır' : 'Repost'}
                    >
                      <Text style={[styles.action, review.reposted && styles.repostedAction]}>
                        ↻
                      </Text>
                      <Text style={styles.count}>{review.reposts ?? 0}</Text>
                    </Pressable>
                  </View>
                        ...
                        return (
                          <View key={comment.id} style={styles.comment}>
                            <View style={styles.commentHeader}>
                              ...
                              {isOwnComment && (
                                <Pressable
                                  onPress={() => deleteComment(review.id, comment.id)}
                                >
                                  <Text style={styles.deleteComment}>🗑️ Sil</Text>
                                </Pressable>
                              )}
...
const baseStyles = StyleSheet.create({
  ...
  storyTapLeft: {
    position: 'absolute', left: 0, top: 0, bottom: 0, width: '42%', zIndex: 10,
  },
  storyTapRight: {
    position: 'absolute', right: 0, top: 0, bottom: 0, width: '58%', zIndex: 10,
  },
  ...
  reviewPostCard: { borderColor: '#34284F', backgroundColor: '#111018' },
  quotePostCard: {
    borderLeftWidth: 3,
  ...
  count: { fontSize: 11, color: '#777F8C' },
  commentBox: {
    marginTop: 11,
    paddingTop: 11,
    borderTopWidth: 1,
