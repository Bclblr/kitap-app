...
export default function EventEditorScreen() {
  ...
  async function save() {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
      setError('Tarihi YYYY-AA-GG biçiminde yaz. Örnek: 2026-09-20');
      return;
    }

    if (!/^\d{2}:\d{2}$/.test(time)) {
      setError('Saati SS:DD biçiminde yaz. Örnek: 19:30');
      return;
    }
    ...
    if (form.image_url.trim() && !/^https:\/\//i.test(form.image_url.trim())) {
      setError('Görsel için HTTPS adresi kullan.');
      return;
    }
    ...
    try {
      ...
      if (authError || !user) {
        throw new Error('Etkinlik oluşturmak için giriş yapmalısın.');
      }
      ...
      setError(
        e instanceof Error && e.message
          ? e.message
          : 'Etkinlik kaydedilemedi. Bağlantını ve yetkilerini kontrol et.'
      );
    } finally {
      lock.current = false;
      setBusy(false);
  ...
  return (
    <ReaderScreen title="Etkinlik oluştur">
      ...
      <Field
        label="Konum"
        placeholder="Kadıköy, İstanbul veya Online"
        value={form.location}
        onChangeText={(location) => setForm((current) => ({ ...current, location }))}
      />
      <Field
        label="Etkinlik görseli (HTTPS adresi, isteğe bağlı)"
        value={form.image_url}
        onChangeText={(image_url) => setForm((current) => ({ ...current, image_url }))}
      />

      <Action
        disabled={busy || !form.title.trim() || !form.date.trim() || !form.time.trim()}
        label={busy ? 'Oluşturuluyor…' : 'Etkinliği oluştur'}
        onPress={() => void save()}
      />
    </ReaderScreen>
  );
