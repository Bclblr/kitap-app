...
export default function StoryCreateScreen() {
  ...
  return <View style={styles.root}>
    <View style={[styles.stage, { marginTop: insets.top + 8 }]}>
      ...
      <View style={styles.topBar}><Pressable onPress={() => safeBack(router, '/')} style={styles.iconButton}><Feather name="x" size={31} color="#fff" /></Pressable>{!capturedUri && !textMode ? <Pressable onPress={() => setFlash(current => current === 'off' ? 'on' : 'off')} style={styles.iconButton}><Feather name={flash === 'on' ? 'zap' : 'zap-off'} size={26} color="#fff" /></Pressable> : <View />}<Pressable onPress={() => setSettingsVisible(true)} style={styles.iconButton} accessibilityRole="button" accessibilityLabel="Hikâye ayarlarını aç"><Feather name="settings" size={26} color="#fff" /></Pressable></View>
